...
class Favoritos
{
    ...
    {
        $sql = "INSERT INTO `favoritos`(`usuario`, `producto`) 
                VALUES ('$user','$product')";
    ...
    {
        $sql = "SELECT * FROM `favoritos` WHERE `usuario` = '$user' AND `producto` = '$product'";
    ...
    function list($filter, $category = false, $subcategory = false, $images = false, $order = '', $limit = '')
    {
        ...
        if ($favoritos) {
            while ($row = mysqli_fetch_assoc($favoritos)) {
                $row =  $this->product->checkPriceByUser($row);
                ...
                $imagesArray = ($images && isset($row["imagenes_rutas"])) ? $this->createArrayImages($row) : '';
                if ($images) unset($row["imagenes_id"], $row["imagenes_orden"], $row["imagenes_rutas"]);
                $array[] = ["data" => $row, "images" => $imagesArray, "combination" => $combinacionData, "link" => $link, "favorite" => ['data' => true]];
            }
            return $array;
        }
    }
