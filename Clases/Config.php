...
class Config
{
    ...
    public function updatePayment()
    {
        ...
        $payments = $this->pagos->list(array("tipo=" . $this->id), '', '');
        if (!empty($payments)) {
    ...
    public function addCheckout($type)
    {
        $checkoutData = $this->viewCheckout($type);
        if (is_array($checkoutData['data'])) {
            ...
            $sql = "INSERT INTO `_cfg_checkout`(`tipo`, `estado`, `mostrar_precio`, `envio`, `pago`) 
                VALUES ({$this->tipo},
                        {$this->estado},
                        {$this->mostrar_precio},
                        {$this->envio},
    ...
    public function viewCheckout($type)
    {
        $this->set("tipo", $type);
        $sql = "SELECT * FROM `_cfg_checkout` WHERE `tipo` = $this->tipo";
        $data = $this->con->sqlReturn($sql);
        $row = mysqli_fetch_assoc($data);
        //seters envios
