...
class EstadosPedidos
{
    ...
    public function add()
    {
        $sql = "INSERT INTO `estados_pedidos`(`estado`, `titulo`, `asunto`, `mensaje`, `enviar`) 
                VALUES ({$this->estado},
                        {$this->titulo},
                        {$this->asunto},
                        {$this->mensaje},
    ...
    {
        $sql = "DELETE FROM `estados_pedidos` WHERE `id` = {$this->id}";
        $query = $this->con->sql($sql);
    ...
    {
        $sql = "SELECT * FROM `estados_pedidos` WHERE id = $id ORDER BY id DESC";
