...
class ProductosRelacionados
{
    ...
    public function add()
    {
        $sql = "INSERT INTO `productos_relacionados`(`cod`,`titulo`, `productos_cod`) 
                  VALUES ({$this->cod},
                          {$this->titulo},
                          {$this->productos_cod})";
    ...
    {
        $sql = "DELETE FROM `productos_relacionados` WHERE `cod`  = {$this->cod}";
        $query = $this->con->sql($sql);
    ...
    {
        $sql = "SELECT * FROM productos_relacionados WHERE   cod = {$this->cod}  ";
    ...
    public function listAdmin($filter)
    {
        $array = array();
        if (!empty($filter)) {
            $filterSql = "WHERE ";
